...
const SidebarMembers = ({ members }) => {
    ...
    <div className="space-y-4">
        {members.map((member) => {
            ...
            return (
              <div key={member.id} className="flex items-center gap-4 p-4 bg-gray-900 rounded-lg">
                {avatarUrl ? (
                  <img className="w-12 h-12 rounded-full" src={`https://anilibria.top${avatarUrl}`} alt={member.nickname} />
                ) : (
                  <svg className="w-12 h-12 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 209 193">
                ...
                <div className="flex flex-col">
                  <strong className="font-medium text-white">{member.nickname}</strong>
                  <span className="text-gray-400 text-sm">{member.role.description}</span>
                </div>
              </div>
            );
          })}
